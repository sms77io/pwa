...
export const BottomNav = () => {
    ...
    const actions: BottomNavigationActionProps[] = [
        {label: 'common:sms', value: 'send', icon: <SmsIcon/>},
        {label: 'lookup:lookup', value: 'lookup', icon: <PolicyIcon/>},
        {label: 'common:options', value: 'options', icon: <SettingsIcon/>},
        {label: 'contacts:contacts', value: 'contacts', icon: <ContactsIcon/>},
        {label: 'pricing:pricing', value: 'pricing', icon: <LocalAtmIcon/>},
    ];
    ...
    >
        {actions.map((a, i) => <BottomNavigationAction
            icon={a.icon} key={i} label={t(a.label as string)} value={a.value}/>)}
    </BottomNavigation>;
};